...
The user's funnel is **Greenhouse and Ashby** — 54 and 22 tracked boards, 16 of 20 applications. Those
...
systems they are not applying through. *(Vendor behaviour changes — if this becomes decision-relevant,
...
their primary stack and absent from the record; put it to them before calling it a gap, per the gap
...
- ✅ The record says they shipped Playwright end-to-end tests gating canary promotion; the resume calls
...
evidence is real, and only once. They already do this well in places (`MCP (Model Context Protocol)`)
and not in others.
...
- **Filename.** Every file in this repo is `resume.pdf`. What a recruiter downloads should be
  `{FirstName}_{LastName}_Resume.pdf`. Flag it if the delivered file is not named that.
...
will happily report a clean vocabulary match on a resume for a role the user should not apply to.

**And it is not the main lever.** As of 2026-08-21: 16 cold portal applications, 0 screens. Both
screens they have ever had came from recruiter inbound. If the cold column stays at zero after this,
