...
1. **Company research.** Check `user/research/{company}.md`. If absent, offer `/company-research
   {company}`. Research is a file, not an endpoint. If he declines, proceed without it.
...
he says to — and not at all for `stretch` or `skip` unless he explicitly asks.
...
Record the batch with `jobhub_db.post_search_results`, then ask which he wants evaluated in depth.
