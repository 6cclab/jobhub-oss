...
5. **The output file must open with that identity block** — canonical domain, one-line description,
   and the collisions ruled out. `user/research/{company}.md` briefs are the
...
and must never be reported as if it were.** Established 2026-08-28 on a consumer-health company, where a 249-review
...
- Interview process notes (search "{company} software engineer interview process") — **and whether
  live coding is algorithmic or practical.** That distinction decides the user's prep and sometimes
...
- **Count churn in the C-suite over 3 years**, with dates. A short tenure is a finding: at one
  company a 20-year big-tech veteran was hired as COO and gone in **under six months**
...
- **Cluster events by month and look for convergence.** The single most valuable finding in that pass came
  from noticing that a CTO hire, a COO hire, a COO exit and an engineering layoff all landed in **May
  2025** — which turned anonymous "they want a big-tech culture" review text into something
...
Established across the 2026-08-28 research pass. These are not style notes; each one exists
...
1. **Label sourced claims and inferences differently, in the text.** "The timing is exact, but no
   source names them — this is inference" is a usable finding. The same sentence without that clause
...
4. **Re-scoping evidence can move a verdict in the company's favour, and that must be reported too.**
   Scoped to engineering, that favoritism finding went from "corroborated fail" to partial,
