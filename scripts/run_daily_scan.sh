...
# The funnel store. JOBHUB_URL is gone: the server was retired 2026-09-11 and
# the funnel is a local SQLite file (docs/sqlite-funnel-design.md). $JOBHUB_DB
# overrides the path for a throwaway run; unset means user/jobhub.db.
STORE="${JOBHUB_DB:-$REPO/user/jobhub.db}"
[[ -f "$STORE" ]] || fail "no funnel store at $STORE — load a boards export from user/boards-export/ first"

log "repo=$REPO work=$WORK"
log "store=$STORE ollama=$OLLAMA_HOST"
...
if [[ "$POST_RC" -ne 0 ]]; then
  log "recording the batch failed; digest written to $DIGEST"
  notify "JobHub scan — store write failed" "$KEPT roles found but NOT recorded in the funnel store. See $DIGEST"
else
