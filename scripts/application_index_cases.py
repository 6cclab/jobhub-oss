...
def _main_table(out):
    """Just the '## All applications' table.

    The index grew an '## In flight' section above it on 2026-08-31, sorted by
    stage rather than date. Date ordering is an invariant of the main table, not
    of the whole document, so assertions about it must be scoped or they test
    the wrong thing.
    """
    return out.split('## All applications', 1)[1]


def case_sorts_newest_first():
    out = _main_table(bai.build(_dir(acme=ACME, globex=GLOBEX)))
...
def case_pipe_in_role_does_not_split_the_row():
    ...
    full = bai.build(_dir(acme=piped))
    # The In-flight section is a second render path for the same free text and
    # has the identical splitting hazard, so assert on BOTH rows, not just the
    # main table's. Added 2026-08-31 when that section was introduced.
    flight_rows = [l for l in full.split('## All applications')[0].split('\n')
                   if 'Backend' in l]
    assert len(flight_rows) == 1, flight_rows
    assert 'Backend \\| Platform Engineer' in flight_rows[0], flight_rows[0]
    assert len(re.findall(r'(?<!\\)\|', flight_rows[0])) == 5, flight_rows[0]

    out = _main_table(full)
...
def case_migrate_refuses_to_parse_the_generated_index():
    ...
    with _MigWorkdir() as w:
        # Derive the widened header from bai.HEADER rather than hardcoding
        # the column list. A hardcoded copy silently stops widening anything
        # the day a column is added -- which is exactly what happened when
        # 'Last' was added on 2026-09-11, disarming this guard's own test.
        lines = []
        for line in bai.HEADER.split('\n'):
...
# --- isolation baseline ------------------------------------------------------
#
# The check itself predates these cases. What is pinned here is the BASELINE
# split added 2026-09-11: 13 pre-existing leaks must stay warnings while a new
# leak fails. A gate that fires on history gets switched off on day one, and a
# baseline that never shrinks is permission nobody re-examines.
...
The user accepted Globex on 2026-09-08, which moves Acme down the pipeline.
"""


def case_isolation_finds_a_leak():
    hits = bai.check_isolation(_dir(acme=LEAKY, globex=GLOBEX))
    assert [h[0] for h in hits] == ['acme'], hits
    assert hits[0][2] == ['Globex'], hits[0]


def case_isolation_ignores_a_path_reference():
    ok = LEAKY.replace('The user accepted Globex on 2026-09-08, which moves Acme '
                       'down the pipeline.',
                       'See `user/applications/globex-platform-engineer.md`.')
    assert bai.check_isolation(_dir(acme=ok, globex=GLOBEX)) == []


def case_mentions_ok_suppresses_a_declared_company():
    declared = LEAKY.replace('applied: 2026-08-24',
                             'applied: 2026-08-24\nmentions_ok: Globex')
    assert bai.check_isolation(_dir(acme=declared, globex=GLOBEX)) == []


def case_baseline_file_parses_comments_and_blanks():
    d = Path(tempfile.mkdtemp())
    f = d / 'baseline.txt'
    f.write_text('# a comment\n\nacme  # Acme: names Globex\n   \nglobex\n')
    old = bai.ISOLATION_BASELINE
    try:
        bai.ISOLATION_BASELINE = f
        assert bai.load_isolation_baseline() == {'acme', 'globex'}
    finally:
        bai.ISOLATION_BASELINE = old


def case_missing_baseline_is_an_empty_set_not_a_crash():
    old = bai.ISOLATION_BASELINE
    try:
        bai.ISOLATION_BASELINE = Path(tempfile.mkdtemp()) / 'nope.txt'
        assert bai.load_isolation_baseline() == set()
    finally:
        bai.ISOLATION_BASELINE = old
