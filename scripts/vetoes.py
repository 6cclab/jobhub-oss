...
def load_closed_companies():
    """Companies vetoed by name, from `user/closed-companies.txt`.

    This is personal data -- which company rejected whom -- so it lives under
    user/ and never ships with the framework. One lowercase name per line;
    blank lines and `#` comments are ignored. A missing file means no
    company-level veto, which is the correct default for a fresh checkout.

    This is belt-and-braces, not the primary mechanism. `scan.py`'s
    load_rejected_companies() already omits any company with a rejected record
    under `user/applications/`, and does it from the authoritative files. This
    veto exists for a company you want held out regardless of what the records
    say, and because a veto is a statement of certainty `appeal.py` can skip.
    """
    path = Path(__file__).resolve().parent.parent / 'user' / 'closed-companies.txt'
    try:
...
CLOSED_COMPANIES = load_closed_companies()
...
# the user had already decided not to apply to. A quantized model asked to weigh
# "fit" against a ban will find strong fits, because strong fit is exactly when
# the ban is tempting. So it is not asked.
#
# The filter is also evidence-backed, not just a preference: one company rejected
# the user from a Staff req saying they expected someone more technical for the level.
...
# Identity and Access Management as the team's CHARTER. The user, 2026-08-21:
...
# "identity" -- the user has real identity experience (event-driven MFA delivery,
