...
# CLOSED_COMPANIES is loaded from user/closed-companies.txt, which is personal
# data and does not exist in CI or in a fresh clone. Pin a known value so this
# file tests the veto mechanism rather than whatever the local user/ happens to
# hold -- without this the closed-company case passes only on the author's
...
CASES = [
    ...
    # already surfaced when the user called it. A bare "Identity" must NOT match --
    # adjacent identity work is still in scope.
    ("Senior Software Engineer 2, IAM", "Globex", "", True),                         # [real]
    ("Senior Backend Engineer, IAM", "Umbrella", "", True),                          # [real]
    ...
    ("Senior Software Engineer", "ClosedCo", "", True),
