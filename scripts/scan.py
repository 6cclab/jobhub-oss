...
Pulls the tracked board list from `user/jobhub.db`, fetches each board plus the
...
def load_boards():
    """Tracked boards from user/jobhub.db. Raises if the store has none.

    This was a GET against the deployed server until 2026-09-11. The boards list
    is the one hand-curated thing in the funnel -- 110 slugs built up over weeks
    -- so an EMPTY result must be loud, not a quiet zero-board scan. See the
    caller, which treats it as fatal.
    """
    conn = db.connect()
    try:
        return [b for b in db.list_boards(conn, status='tracked') if b.get('slug')]
    finally:
        conn.close()


def load_rejected_companies():
    """Companies that already rejected the candidate, from user/applications/.

    READ FROM THE FILES, which are authoritative. This used to GET
    /api/applications?status=rejected, and on 2026-09-11 that was measured
    returning 8 companies where the records held 11 -- three were missing, so
    their postings were eligible to resurface in every scan.
    The applications table stopped being written on 2026-08-26 when applications
    moved to files; the filter kept reading it and nothing said so, because the
    except-branch below turns any failure into an empty set.

    These are OMITTED, not flagged. The user, 2026-08-21: "if previously rejected
    then omit." An earlier version surfaced them with a warning marker and they
    asked for them gone.
    ...
    """
    rejected = set()
    for path in sorted((REPO / 'user' / 'applications').glob('*.md')):
        try:
            rec = ar.load(path)
        except Exception as e:  # noqa: BLE001 - one bad record must not blind the filter
            print(f'  warning: could not read {path.name} for the rejection filter'
                  f' -- {type(e).__name__}: {e}', file=sys.stderr)
            continue
        if (rec.get('status') or '').strip().lower() == 'rejected':
...
# The user, 2026-08-21: "we should never re-fetch listings within 30 days."
...
def record_seen(postings, when, reason=None):
    ...
    that vanished with it could never reappear in a future scan. The user:
    "no silent losses. If I've seen it then dont go get it again."

    So the ledger now means "this reached the user", not "this was fetched". A crash
...
def forget(substr):
    ...
    would resurface things the user has already reviewed.
    """
    if not SEEN.exists():
        return 0
    keep, dropped = [], 0
...
def main():
    ...
    try:
        ...
        print(f'FATAL: could not read boards from {db.DEFAULT_PATH}'
              f' -- {type(e).__name__}: {e}', file=sys.stderr)
        return 1

    if not boards:
        # An empty store looks exactly like a successful scan of nothing. It is
        # not: it means the curated list is missing. Restore it with
        #   python3 -c "import sys;sys.path.insert(0,'scripts');import jobhub_db as d;
        #               d.load_boards_export(d.connect(),'user/boards-export/boards-<date>.json')"
        print(f'FATAL: no tracked boards in {db.DEFAULT_PATH}. Load an export from'
              ' user/boards-export/ before scanning.', file=sys.stderr)
        return 1

    jobs = [(b.get('ats'), b['slug'], b.get('name')) for b in boards]
    print(f'{len(jobs)} boards from {db.DEFAULT_PATH}'
    ...
    # run, which is the re-fetching the user asked to stop. They are not a silent
