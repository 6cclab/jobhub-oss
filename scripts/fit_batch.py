#!/usr/bin/env python3
"""Fit-report a screened queue against the user's actual record.
...
flags against his preferences, and a verdict -- and records it in the funnel
store, so the ranking is built on his record rather than on a title match.

The store is `user/jobhub.db` (docs/sqlite-funnel-design.md). This used to POST
to `/api/fit-reports` on a deployed server, retired 2026-09-11.
...
def post_fit_report(role, v, conn=None):
    """Record one fit report. Returns (id_or_None, error_or_None)."""
    body = {
    ...
    if own:
        conn = db.connect()
    try:
        return db.create_fit_report(conn, body), None
    except Exception as e:  # noqa: BLE001 - caller records the error per role
...
def main():
    ...
    if not a.no_post:
        posted = 0
        # One connection for the whole run. The old HTTP path slept 0.05s between
        # posts to be kind to the server; a local file needs no such courtesy.
        conn = db.connect()
        try:
            for r in results:
                fid, err = post_fit_report(r, r, conn=conn)
                if fid:
                    r['fit_report_id'] = fid
                    posted += 1
                else:
            ...
            conn.close()
        print(f'recorded {posted}/{len(results)} fit reports in {db.DEFAULT_PATH}',
              flush=True)
        if posted < len(results):
            # Surface it here, not only in the JSON nobody opens on a good day.
            for r in results:
                if r.get('post_error'):
                    print(f'  FAILED {r.get("company")} — {r["post_error"]}',
                          file=sys.stderr)
