...
def _():
    got = carveouts_from({'rejections': [
        {'company': "O'Neill & Sons", 'exclude': False, 'reason': 'role filled'}]})
    assert got == {"o'neill & sons": (False, 'role filled')}, got


@case('exclude_true_stays_excluded')
def _():
    got = carveouts_from({'rejections': [
        {'company': 'Initech', 'exclude': True, 'reason': 'assessed and passed'}]})
    assert got['initech'][0] is True, got
...
def _():
    ...
    rejected = {'globex', 'umbrella', 'acme'}
    carve = {'acme': (False, 'role filled'),
             'initech': (True, 'assessed and passed'),
    ...
    assert kept == [('acme', 'role filled')], kept
    assert remaining == {'globex', 'umbrella'}, remaining


@case('live_rejections_json_parses_when_present')
def _():
    # The real file, not a fixture -- catches a hand-edit that breaks the schema.
    #
    # SKIPPED when absent, and it asserts on shape rather than on which companies
    # are listed. user/ is personal data: it does not exist in CI or in a fresh
    # clone, and naming real companies here would both leak them and make this
    # suite pass only on one machine. That is exactly how this case sat red in
    # CI while passing locally.
    if not (REPO / 'user' / 'rejections.json').exists():
        return
    got = scan.load_rejection_carveouts()
    assert got, 'user/rejections.json exists but produced no entries'
    for name, value in got.items():
        assert name == name.strip().lower(), f'company not normalised: {name!r}'
        exclude, reason = value
        assert isinstance(exclude, bool), (name, exclude)
        assert isinstance(reason, str), (name, reason)
        if exclude is False:
            assert reason, f'{name}: exclude:false must carry a reason'
