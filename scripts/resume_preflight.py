...
same session that wrote them. The user, 2026-08-20: "you keep leaving notes and then
...
Checks, in order:
  ...
  PROVENANCE  -- the master resume is the law: every claim cites the line of
                 master-resume.md it derives from, every figure and term it asserts
                 appears in those cited lines, and a change to the law makes the
                 citation stale (user/provenance-grandfathered.txt exempts the archive)
...
def check_prose(md):
    ...
    sentence nobody can parse -- the user, 2026-08-20, on 'I contributed to consolidating
    ...
    # {employer}". Employment dates already live in Experience, where they are verifiable;
    ...
    # Subsection labels inside Experience. The user, 2026-08-21: "Again adding boldened sub
    # sections" -- "again" because they had said it before and nothing was written down, so it
    ...
    # They chose to drop them entirely rather than unbold them: bullets sit under the company
    ...
    # which is the exact form the user complained about, so the guard would have let the
...
def skills_of(md):
    """Just the Skills block. The inverse of prose_of().

    check_claims() deliberately excludes this block -- running verb-escalation over
    an inventory only ever produced false positives. But an inventory makes its own
    kind of claim: a languages line asserts working proficiency. That needs a check
    of its own, not a rule smuggled into the prose bucket where it can never fire.
    """
    if "## Skills" not in md:
        return ""
    rest = md.split("## Skills", 1)[1]
    return rest.split("\n## ", 1)[0]


def check_skills(md, rules):
    """Terms that must not appear in the Skills block.

    Added 2026-09-11, after the user disclosed that the Go in personal-projects.md is
    agent-produced and they cannot write or debug it unaided. Go was on ten resumes'
    Languages line, two of them still live applications. A skills line is the one
    place a claim is made with no verb and no context to qualify it, which is
    exactly why it needs a gate rather than a judgement call.
    """
    v, block = [], skills_of(md)
    if not block:
        return v
    low = block.lower()
    for pat, why in rules.get("forbidden_skills", []):
        for m in re.finditer(rf"\b(?:{pat})\b", low):
            v.append(("FORBIDDEN_SKILL", ctx(block, m.start(), m.end()), why))
    return v
...
def check_review(d: Path, md_text):
    ...
    the user rather than silent. That matters because the review loop has been marked
    MANDATORY in resume-style.md since 2026-08-09 and was skipped on all three resumes
    built on 2026-08-20 -- the user: "Ok, so how do we enforce it?"
...
def check_artifact(d: Path):
    ...
    # bold, which matched no branch, so two PDFs shipped with
...
# Location on a resume is ALWAYS "New Jersey" -- never a town, never a NYC address, on any
# field or document. Decided 2026-08-18 (job-auto.md) and explicitly not a per-role call.
#
# This is checked over the WHOLE document, not prose_of(), because that is the entire point:
# on 2026-08-28 the home town + state was found on five resumes and four already-submitted
# applications, sitting in the contact header -- which prose_of() strips, so every
# claim rule ran right past it. A rule in claim-rules.json would not have fired either.
HOME_LOCATION = re.compile(
    r"\b(sewell|mantua|glassboro|woodbury|deptford|turnersville|sicklerville)\b"
    r"|\b[A-Z][a-z]+\s*,\s*(NJ|NY|New Jersey|New York)\s*\d{5}"
    r"|\b\d+\s+[A-Z][a-z]+\s+(St|Street|Ave|Avenue|Rd|Road|Ln|Lane|Dr|Drive)\b",
    re.I)


def check_location(md):
    v = []
    for m in HOME_LOCATION.finditer(md):
        if re.fullmatch(r"new jersey", m.group(0).strip(), re.I):
            continue
        v.append(("HOME_LOCATION", ctx(md, m.start(), m.end()),
                  "location is always 'New Jersey' -- never a town, never a street address, "
                  "never a NYC address, on any field or document (decided 2026-08-18)"))
    return v


def check_gaps(missing, rules):
    """A term is only a gap if BOTH source files lack it. Anything outside the
    primary stack must be put to the user before it is called a gap at all."""
    out = []
    src = {}
    ...
    for term in missing:
        ...
        if hits:
            ...
            out.append(("MUST_ASK", term,
                        "outside the primary stack and absent from both files -- ASK the user before "
                        "calling it a gap. Absence from the record is not absence from the user."))
    return out


def check_provenance_gate(d: Path):
    """The master resume is the law. Delegates to scripts/check_provenance.py.

    Every claim on a tailored resume must cite the line of master-resume.md it derives
    from, and every figure and term it asserts must appear in the cited lines. Added
    2026-08-28 after an LLM review judge asserted a TRUE claim was a conflation, the
    finding was accepted without reading the cited line, and the claim was stripped from
    two resumes plus written into claim-rules.json as a permanent rule. Every gate stayed
    green, because all of them checked what was ADDED and none checked whether what
    remained still traced to the record.

    Directories listed in user/provenance-grandfathered.txt predate the gate and report
    rather than fail. That list is meant to shrink to zero; do not add to it.
    """
    exempt = ROOT / "user" / "provenance-grandfathered.txt"
    label = d.relative_to(ROOT / "user" / "tailored").as_posix()
    if exempt.exists():
        skip = {l.strip() for l in exempt.read_text().split("\n")
                if l.strip() and not l.startswith("#")}
        if label in skip and not (d / "provenance.json").exists():
            return [("GRANDFATHERED", "provenance.json missing",
                     "predates the provenance gate -- exempt until this resume is next edited. "
                     "Retrofit it with: python3 scripts/check_provenance.py " + label)]
    spec = importlib.util.spec_from_file_location("cp", ROOT / "scripts" / "check_provenance.py")
...
def run(d: Path, missing, rules):
    ...
    v += check_skills(raw, rules)
    v += check_prose(raw)
    v += check_review(d, raw)
    v += check_artifact(d)
    v += check_location(raw)
    v += check_provenance_gate(d)
    ...
    soft = {"MUST_ASK", "GRANDFATHERED"}
    hard = [x for x in v if x[0] not in soft]
    print(f"{RED if hard else YEL}{'FAIL' if hard else 'ASK '}{OFF}  {label}  ({len(v)} finding(s))")
    for kind, quote, why in v:
        colour = YEL if kind in soft else RED
        print(f"    {colour}[{kind}]{OFF} {quote[:88]}")
        print(f"        {DIM}{why}{OFF}")
    # GRANDFATHERED alone must not break the archive -- it is a standing to-do, not a
    # defect in the resume. MUST_ASK keeps its original non-zero exit.
    return 1 if (hard or any(x[0] == "MUST_ASK" for x in v)) else 0


def sent_records():
    """Dirs whose resume was actually SENT. They are immutable records, not working documents.

    Editing one changes nothing -- the employer already has that PDF -- so re-reporting it on
    every --all forever is noise that buries live findings. Added 2026-08-28, when a stale
    "7+ years" figure was found on 30 resumes and the 18 already-submitted ones made the other
    12 (which are still sendable, and still wrong) hard to see.

    Deliberately keyed on `submitted: true` PLUS a `resume:` path, not on status. A packet that
    was built but never sent stays in the sweep, which is the whole point.
    """
    out = set()
    apps = ROOT / "user" / "applications"
    if not apps.exists():
        return out
    for f in apps.glob("*.md"):
        txt = f.read_text()
        if not re.search(r"^submitted:\s*true\s*$", txt, re.M):
            continue
        m = re.search(r"^resume:\s*(.+)$", txt, re.M)
        if m:
            out.add(str(pathlib.PurePosixPath(m.group(1).strip().strip('"')).parent))
    return out
...
def main():
    ...
    if "--all" in args:
        sent = sent_records()
        allr = sorted(p.parent for p in (ROOT / "user" / "tailored").rglob("resume.md")
                      if "_archive" not in p.parts)
        dirs = [d for d in allr
                if d.relative_to(ROOT / "user" / "tailored").as_posix() not in sent]
        if len(allr) != len(dirs):
            print(f"{DIM}{len(allr) - len(dirs)} sent record(s) skipped -- already submitted, "
                  f"immutable. Pass a dir explicitly to check one.{OFF}")
    elif args:
